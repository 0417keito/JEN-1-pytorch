...
from utils.script_util import save_checkpoint, TASK
...
class UnifiedMultiTaskTrainer(nn.Module):
    ...
    def eval_all_tasks(self, rank=0):
        ...
        with tqdm(self.tasks, desc="Evaluating Tasks") as pbar:
            for task in pbar:
                ...
                pbar.set_postfix({'avg_loss': task_loss / task_count if task_count else 0})
        ...
        self.avg_total_loss = avg_total_loss
    ...
    def eval(self, task, rank=0):
        ...
        with torch.no_grad(), tqdm(enumerate(self.valid_dl), desc=f"Validating: {task}",
                                   total=len(self.valid_dl)) as pbar:
            for batch_idx, (audio_emb, metadata) in pbar:
    ...
    def train_loop(self):
        ...
        for epoch in range(self.epoch_str, int(num_epoch + 1)):
            ...
            with tqdm(total=batches_per_task, desc=f"Epoch {epoch}/{num_epoch}") as pbar:
                for i in range(batches_per_task):
                    weighted_loss = 0.0  # Keep this as a tensor
                    ...
                    for task in self.tasks:
                        ...
                        weighted_loss += loss  # Do not convert to float here
                        loss_dict[task] = loss.item()  # Store the item for logging
    ...
    def random_mask(self, sequence, max_mask_length, task):
        ...
        if task.lower() == TASK.TEXT_GUIDED.value:
            ...
            masks.append(mask)
            causal = False
        elif task.lower() == TASK.MUSIC_CONT.value:
            mask_length = random.randint(sequence_length * 0.2, sequence_length * 0.8)
