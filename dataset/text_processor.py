...
class TextProcessor:
    ...
    def clean(self, text, cleaner_names=['cje_cleaners']):
        import g2p.cleaners
        for name in cleaner_names:
            cleaner = getattr(g2p.cleaners, name)
            if not cleaner:
    ...
    def phonemize(self, phoneme_tokens):
        phoneme = self.tokenizer.decode(phoneme_tokens)
        return phoneme
